...
const chooseDisplayImage = (url,event) =>{
  ...
  return (
    ...
    {location.state.work.image_urls.internal_image.map((eachImgUrl)=>{
      if (counter==0){
        counter=counter+1;
        return  <li key={eachImgUrl} className="active" ref={getDom} onClick={(e)=>chooseDisplayImage(eachImgUrl,e)}></li>  
      } else {
        counter=counter+1;
        return  <li key={eachImgUrl} onClick={(e)=>chooseDisplayImage(eachImgUrl,e) }></li> 
      } 
    }
    )}
    </ul>
